...
export default function DocumentReviewView({ file, onBack, onApprove }: DocumentReviewViewProps) {
  return (
    <div className="absolute inset-0 bg-[#f8fafc] flex flex-col z-20 w-full h-full">
      ...
      <header className="h-auto sm:h-[52px] border-b border-slate-200 bg-white flex items-center justify-between px-3 sm:px-4 py-2 sm:py-0 flex-shrink-0 shadow-sm gap-3">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          ...
          <div className="flex items-center gap-2 min-w-0">
             ...
             <span className="font-semibold text-slate-900 text-[15px] tracking-tight truncate">{file.name}</span>
          </div>
          <div className="hidden sm:block w-px h-5 bg-slate-300 ml-1"></div>
          <span className="hidden sm:inline text-[13px] font-medium text-slate-500 ml-1">Page 1 of 4</span>
        </div>
        
        <div className="flex items-center gap-2.5 text-slate-500 mr-0 sm:mr-2 flex-shrink-0">
      ...
      <div className="flex-1 overflow-hidden flex flex-col lg:flex-row min-h-0 bg-[#eef1f4]">
        {/* Left Side: Document Viewer */}
        <div className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto flex justify-center custom-scrollbar">
            <div className="max-w-[750px] w-full bg-white shadow-lg border border-slate-300 relative h-max min-h-[520px] sm:min-h-[760px] lg:min-h-[1000px]">
               <img src={file.imageUrl} alt="Document" className="w-full h-full object-cover object-top opacity-90 blur-[1px] p-6 sm:p-10 md:p-16" style={{ filter: 'grayscale(0.3) blur(0.5px)' }} />
               <div className="absolute inset-0 bg-white/20 pointer-events-none mix-blend-overlay"></div>
            </div>
        </div>

        {/* Right Side: Analysis Panel */}
        <div className="w-full lg:w-[360px] bg-white border-t border-slate-200 lg:border-t-0 lg:border-l flex flex-col flex-shrink-0 z-10 relative">
          ...
          <div className="p-4 border-t border-slate-200 bg-white flex flex-col sm:flex-row gap-3 flex-shrink-0 z-20">
             <button className="px-5 bg-white border border-slate-200 hover:border-slate-300 hover:bg-slate-50 text-slate-700 font-medium rounded-md flex items-center justify-center gap-2 transition-colors cursor-pointer shadow-sm text-[14px] py-2.5 w-full sm:w-auto">
